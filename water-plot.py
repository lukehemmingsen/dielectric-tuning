...
def new_files(con, HFcon, B3con, BHHcon):
    gen.clear_ins()
    gen.clear_outs()
    gen.generate_ins(con, HFcon, B3con, BHHcon)
    os.system("qchem water.in > water.out")
    os.system("qchem watera.in > watera.out")
    os.system("qchem waterHF.in > waterHF.out")
    os.system("qchem waterHFa.in > waterHFa.out")
    os.system("qchem waterB3LYP.in > waterB3LYP.out")
    os.system("qchem waterB3LYPa.in > waterB3LYPa.out")
    os.system("qchem waterBHHLYP.in > waterBHHLYP.out")
    os.system("qchem waterBHHLYPa.in > waterBHHLYPa.out")

def sub_wait():
    print "Queued at " + time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
    hasPath =  os.path.exists(path + '/waterBHHLYPa.out')
    somethingIsWrong = 0
    while not hasPath:
	time.sleep(1)
	hasPath = os.path.exists(path + '/waterBHHLYPa.out')
...
def generate(con, HFcon, B3con, BHHcon):
    new_files(con,HFcon,B3con,BHHcon)
    if not sub_wait() :
...
class GraphResult:
	...
	plt.plot(X, self.BHHLYP.q, 'g', lw=8)
        ...
        plt.xlabel('Charge on equilibrium water structure', fontsize=48, fontweight='bold')
...
class Parameters:
    ...
    def generate(self):
	return generate(self.con, self.HFcon, self.B3con, self.BHHcon)
